<template>
  <view class="site">
	  <view class="padding-top-xl">导入视频源:</view>
    <view class="impSite" maxlength="300" >
      <u-input v-model="url" type="text" border="true" placeholder="请输入视频源网址"/>
...
export default {
  ...
  methods: {
    ...
    async importSiteEvent () {
      ...
      const data={
		      "id": this.siteList.length+1,
		      "key": "drsp"+this.siteList.length+1,
		      "name": "导入视频源"+this.siteList.length+1,
		      "api": this.url,
		      "download": "",
		      "group": "默认",
		      "isActive": true,
        ...
        this.getAllSite();
        // this.impSite = false
		const ress = await db.getAll("site");
		if (ress.flag) {
