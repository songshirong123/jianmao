...
export default {
  ...
  methods: {
    ...
    async getDetail() {
     ...
     await http.getT('https://pcw-api.iqiyi.com/albums/album/avlistinfo',{aid:this.url,page:'1',size:'100'}).then(res=>{
		  const arr = [];
		  for (const i of res.data.epsodelist) {
		    let label = res.data.epsodelist.length > 0 ? `第${i.order}集`: ''
